  ...
  function Logger(_ref) {
    ...
    var formatObject = function formatObject(obj, depthLevel) {
      ...
      forEachEnumerableOwnProperty(obj, function (key) {
        var value = obj[key];
        if (whitelist.length === 0 || getValueByPath(optimizedWhitelist, [].concat(_toConsumableArray(whiteListPath), [key]))) {
          if (value && _typeof(value) === 'object') {
            formatted[key] = formatObject(value, depthLevel + 1, [].concat(_toConsumableArray(whiteListPath), [key]));
          } else {
